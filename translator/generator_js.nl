...
def print_command(module : ptd::sim(), command : @nlasm::cmd_t) {
	...
	match (command->cmd) case :arr_decl(var arr_decl) {
		...
		result = print_register_to_assign(ov_is->dest) . 'c_rt_lib0ov_is(' . print_register(ov_is->src) . 
			', new ImRef("' . escape_string(ov_is->type) . '"));';
	} case :ov_as(var ov_as) {
		result = print_register_to_assign(ov_as->dest) . 'c_rt_lib0ov_as(' . print_register(ov_as->src) . 
			', new ImRef("' . escape_string(ov_as->type) . '"));';
		...
		result = print_register_to_assign(get_frm_idx->dest) . print_register_value(get_frm_idx->src) . '[Math.floor(' . 
			print_register_value(get_frm_idx->idx) . ')];';
	} case :set_at_idx(var set_at_idx) {
		result = print_register_to_assign(set_at_idx->src) . print_register_value(set_at_idx->src) . '.slice();';
		result .= print_register_value(set_at_idx->src) . '[Math.floor(' . print_register_value(set_at_idx->idx) . 
			')] = ' . print_register_value(set_at_idx->val) . ';';
	} case :get_val(var get_val) {
		result = print_register_to_assign(get_val->dest) . print_register_value(get_val->src) . '["' . 
		...
		result .= print_register_value(set_val->src) . '["' . escape_string(set_val->key) . '"] = ' . 
			print_register_value(set_val->val) . ';';
...
def print_bin_op(bin_op) : ptd::sim() {
	...
	if (bin_op->op eq '>=' || bin_op->op eq '<=' || bin_op->op eq '<' || bin_op->op eq '>' || bin_op->op eq '==' || 
		bin_op->op eq '!=') {
		return result . 'c_rt_lib0native_to_nl(parseFloat(' . print_register_value(bin_op->left) . ') ' . bin_op->op . 
			' parseFloat(' . print_register_value(bin_op->right) . '));';
		...
		return result . print_register_value(bin_op->left) . '.toString() + ' . print_register_value(bin_op->right) . 
			'.toString();';
	} else {
		return result . 'parseFloat(' . print_register_value(bin_op->left) . ') ' . bin_op->op . ' parseFloat(' . 
			print_register_value(bin_op->right) . ');';
	}
}
