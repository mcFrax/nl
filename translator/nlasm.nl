...
def nlasm::cmd_t() {
	return ptd::rec({
			debug => @nlasm::debug_t,
			annotation => ptd::var({none => ptd::none(), const => ptd::arr(ptd::sim())}),
			cmd => @nlasm::order_t
		});
}
...
def nlasm::order_t() {
	return ptd::var({
			...
			call => @nlasm::call_t,
			una_op => ptd::rec({dest => @nlasm::reg_t, src => @nlasm::reg_t, op => ptd::sim()}),
			bin_op => ptd::rec({dest => @nlasm::reg_t, left => @nlasm::reg_t, right => @nlasm::reg_t, op => ptd::sim()}),
			...
			ov_mk => @nlasm::ov_mk_t,
			prt_lbl => ptd::sim(),
			if_goto => ptd::rec({dest => ptd::sim(), src => @nlasm::reg_t}),
			goto => ptd::sim(),
			clear => @nlasm::reg_t
		});
}
