...
def print_hash_declaration(hash : ptd::arr(@nast::hash_elem_t), destination : @nlasm::reg_t, ref state : 
	...
	var elems_array = [];
	fora var elem (hash) {
		array::push(ref elems_array, {key => elem->key as :hash_key, val => calc_val(elem->val, ref state)});
	}
...
def print_die(value : ptd::arr(@nast::value_t), ref state : @translator::state_t) {
	var arg = new_register(ref state);
