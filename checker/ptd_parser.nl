...
def ptd_parser::try_value_to_ptd(ast : @nast::value_t) : ptd::var({ok => @tct::meta_type, err => ptd::sim()}) {
	if (ast is :unary_op) {
		...
		return :err('ptd function argument can be other ptd function or reference type start witch @: ' . op->op)
			unless op->op eq '@';
		var fun_str = op->val as :fun_label;
	...
	if (fun_name eq 'rec') {
		return :err('rec must have hash: ' . ov::get_element(fun_arg)) unless fun_arg is :hash_decl;
		...
		return :err('var must have hash: ' . ov::get_element(fun_arg)) unless fun_arg is :hash_decl;
		var hash_decl = fun_arg as :hash_decl;
		var elems = {};
		fora var hash_elem (hash_decl) {
			if (hash_elem->val is :fun_val) {
				fun_val = hash_elem->val as :fun_val;
				if (fun_val->module eq 'ptd' && fun_val->name eq 'none') {
					hash::set_value(ref elems, hash_elem->key as :hash_key, tct::none());
					continue;
				}
			}
			try var ret = ptd_parser::try_value_to_ptd(hash_elem->val);
			hash::set_value(ref elems, hash_elem->key as :hash_key, ret);
		}
