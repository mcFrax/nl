	...
	match (val) case :ternary_op(var ternary_op) {
		...
		if (ptd_system::is_accepted(left_type2, tct::var({}), ref modules, ref errors)) {
			...
			if (!hash::has_key(variants, var_op->case)) {
				add_error(ref errors, 'case ' . var_op->case . ' don''t exists is: ' . 
			...
			if (is_spec) {
				...
				if (fun_def_arg->mod is :ref) {
					...
					if (err_len == array::len(errors->errors)) {
						...
						var t = {type => fun_def_arg->type, src => :known};
						set_type_to_lval(fun_val_arg->val, args_values_types[i], t, ref modules, ref vars, ref errors);
					}
				}
		...
		ret = check_special_function({type => fun_def->ret_type, src => :known}, fun_val, args_values_types, ref modules, 
				ref vars, ref errors);
	} case :string(var str) {
		ret->type = tct::sim();
...
def unary_op_dec_inc(type : @nast::value_t, err_str : ptd::sim(), ref modules : @tc_types::modules_t, ref vars : 
	...
	vtype->type = tct::sim();
	var err_left_len = array::len(errors->errors);
	var left_type : @tc_types::type = get_type_left_site_equation(type, ref modules, ref vars, ref errors);
...
def get_type_left_site_equation(left : @nast::value_t, ref modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, 
	ref errors : @tc_types::errors_t) : @tc_types::type {
	if (left is :bin_op && (left as :bin_op)->op eq '->') {
