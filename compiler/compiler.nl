...
#		return false;
	...
	if (!(opt_cli->language is :ast || opt_cli->language is :nl)) {
		...
		var tc_errors = newtc::check_modules(modules);
		if (array::len(tc_errors) > 0) {
			fora var error (tc_errors) {
				nl::print(error);
			}
			return false;
		}
...
def check_modules(asts : ptd::hash(@nast::module_t), ref errors : @compiler::errors_group_t, deref : @compiler::deref_t) 
	: ptd::void() {
